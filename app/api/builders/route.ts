  ...
  try {
    ...
    if (!session || !session.user) {
      return NextResponse.json(
        { status: false, builders: [], message: "Não encontramos a sessão do usuario" },
        { status: 401 }
      );
    }
...
export async function POST(req: Request) {
  try {
    const { name, tokenBuilder, tokenRooter, url } = await req.json();
    ...
    if (builder) {
      await createBuilderMember({ userId: user.id, idBuilder: builder.id });
    }
